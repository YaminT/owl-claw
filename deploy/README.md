# OwlClaw — Remote deployment

This directory contains the artifacts needed to deploy OwlClaw to a Linux server. The
...
| `owl-claw.service`    | systemd unit for promoting the tmux-based install to a system service with auto-restart on reboot. Requires sudo on the target. |
...
2. **Surveyed the box** — confirmed: git, curl, rsync, tmux, python3, systemctl present; bun, node, claude, codex absent; port 8090 free; no existing OwlClaw install.
...
4. **Rsynced the repo** to `~/owl-claw/` on the target (excluded `node_modules`, `instructions/`, `bun.lockb`, `*.log`, `.DS_Store`).
5. **Ran `bun install`** on the target; verified `bun build src/index.ts` produces a clean 39 KB bundle.
6. **Created runtime paths:**
   - `~/owl-claw-ins/` — the instructions directory (initially empty).
   - `~/frontend-target/` — a placeholder git repo with a stub `CLAUDE.md`. Replace with the real frontend repo when wiring OwlClaw to an actual codebase.
7. **Started the app in a detached tmux session** named `owl-claw`, logging combined stdout/stderr to `~/owl-claw.log`, with `OWLCLAW_HOST=0.0.0.0` and `OWLCLAW_PORT=8090`.
...
| --- | --- | --- |
| `OWLCLAW_PORT` | `8090` | Remote HTTP port. |
| `OWLCLAW_REMOTE_HOME` | `/home/$USER_FROM_TARGET` | Remote home directory (inferred from `user@host`). |

After a successful run the script prints:

```
>>> UI:      http://49.13.25.232:8090
>>> log:     ssh batman@49.13.25.232 'tail -f ~/owl-claw.log'
>>> attach:  ssh -t batman@49.13.25.232 tmux attach -t owl-claw
>>> stop:    ssh batman@49.13.25.232 tmux kill-session -t owl-claw
```
...
```sh
sudo cp ~/owl-claw/deploy/owl-claw.service /etc/systemd/system/owl-claw.service
sudo systemctl daemon-reload
sudo systemctl enable --now owl-claw
# Stop the tmux session so both aren't trying to bind to 8090 at once:
tmux kill-session -t owl-claw 2>/dev/null || true
sudo systemctl status owl-claw --no-pager
tail -f ~/owl-claw.log
```

Rollback: `sudo systemctl disable --now owl-claw` + restart the tmux session via `./deploy/remote-install.sh`.
...
```sh
# Edit the env var in either the tmux start command or /etc/systemd/system/owl-claw.service:
OWLCLAW_FRONTEND_DIR=/path/to/real/frontend
...
And bind OwlClaw to loopback only:

```
OWLCLAW_HOST=127.0.0.1
```

## Current server state (as deployed)

- OwlClaw 0.1.0 running under `tmux` session `owl-claw` on `batman@49.13.25.232:8090`, exposed to the internet.
- Instructions directory: `/home/batman/owl-claw-ins/` (empty).
